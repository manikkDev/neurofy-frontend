...
export function Sidebar({ variant = "public" }: SidebarProps) {
  ...
  return (
    <aside className="w-64 bg-surface-raised border-r border-surface-border min-h-screen flex flex-col" aria-label="Main navigation">
      <div className="p-5 border-b border-surface-border">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 bg-brand-600 rounded-lg flex items-center justify-center" aria-hidden="true">
            <svg
          ...
          <div>
            <h2 className="text-lg font-bold text-white">Neurofy</h2>
            <p className="text-xs text-gray-500">Tremor Monitor</p>
          </div>
        </div>
      </div>

      <nav className="flex-1 p-3" aria-label={`${variant} navigation menu`}>
          ...
          {navItems.map((item) => (
            <li key={item.to}>
              <NavLink
                to={item.to}
                className={({ isActive }) =>
                  `flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors duration-150 focus:outline-none focus:ring-2 focus:ring-brand-500 focus:ring-offset-2 focus:ring-offset-surface-raised ${
                    isActive
      ...
      <div className="p-4 border-t border-surface-border">
        <p className="text-xs text-gray-600 text-center" role="contentinfo">Neurofy v0.1.0</p>
      </div>
    </aside>
  );
}
